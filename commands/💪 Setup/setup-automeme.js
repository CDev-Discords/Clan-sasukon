  ...
  run: async (client, message, args, cmduser, text, prefix, player, es, ls, GuildSettings) => {
    ...
    try {
      ...
      async function first_layer(){
        ...
        .setColor(es.color)
        .setAuthor(client.getAuthor('Auto MEME Setup', 'https://emojipedia-us.s3.dualstack.us-west-1.amazonaws.com/thumbs/120/whatsapp/302/clown-face_1f921.png', 'https://discord.gg/milrato'))
      ...
      async function handle_the_picks(optionhandletype, menuoptiondata) {
        switch (optionhandletype){
          case "Enable Auto-Meme": { 
            ...
            await tempmsg.channel.awaitMessages({filter: m => m.author.id == message.author?.id, 
              ...
              .then(async collected => {
                ...
                if(channel){
                  await client.settings.set(message.guild.id+`.automeme`, channel.id)
          ...
          case "Disable Auto-Meme": { 
            await client.settings.set(message.guild.id+`.automeme`, "no")
            return message.reply({embeds: [new Discord.MessageEmbed()
          ...
          case "Show Settings": { 
            let thesettings = await get(message.guild.id+`.automeme`)
            return message.reply({embeds: [new Discord.MessageEmbed()
