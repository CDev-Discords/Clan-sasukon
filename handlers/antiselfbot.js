...
module.exports = async (client) => {
  ...
  async function checkSelfBot(client, message, guild_settings) {
    ...
    try {
      ...
      let member = member || message.guild.members.cache.get(message.author.id) || await message.guild.members.fetch(message.author.id).catch(() => null);
      if(!member) return 
      ...
      try {
        if (autowarn.antiselfbot) {
          ...
          message.channel.send({
            embeds: [
              new MessageEmbed().setAuthor(client.getAuthor(message.author.tag, member.displayAvatarURL({ dynamic: true })))
          ...
          for (const role of warnsettings.roles) {
            if (role.warncount == warnings.length) {
              if (!member.roles.cache.has(role.roleid)) {
                member.roles.add(role.roleid).catch((O) => { })
              }
            }
          }
        }
