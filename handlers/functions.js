...
async function send_roster_msg(client, guild, the_roster_db, pre) {
  ...
  await the_roster_db.ensure(guild.id, obj)
  let es = await client.settings.get(guild.id+".embed")
  let ls = await client.settings.get(guild.id+".language")
  let guildData = await the_roster_db.get(guild.id);
...
async function databasing(client, guildid, userid) {
  ...
  let ress = [], respones;
    try {
      if (guildid) {
        respones = await client.customcommands.ensure(guildid, {
          commands: []
        }).catch(() => null)
        ...
        respones = await client.keyword.ensure(guildid, {
          commands: []
        }).catch(() => null)
         ...
         * @INFO
         */
        respones = await client.social_log.ensure(guildid, {
          tiktok: {
            channels: [],
            dc_channel: ""
          },
        ...
        respones = await client.stats.ensure(guildid, {
          commands: 0,
          songs: 0
        }).catch(() => null)
        ...
        respones = await client.premium.ensure(guildid, {
          enabled: false,
        }).catch(() => null)
        ...
        respones = await client.setups.ensure(guildid, ensureData).catch(() => null)

        if(respones && respones.changed) await delay(5);
        ress.push(respones)

        respones = await client.blacklist.ensure(guildid, {
          words: [],
          mute_amount: 5,
          whitelistedchannels: [],
        }).catch(() => null);
        ...
        respones = await client.settings.ensure(guildid, {
   ...
   */
        ...
        respones = await client.jtcsettings.ensure(guildid, {
          prefix: ".",
          channel: "",
        ...
        respones = await client.musicsettings.ensure(guildid, {"channel": "","message": ""})

        if(respones && respones.changed) await delay(5);
        ress.push(respones)

        respones = await client.stats.ensure(guildid, {commands: 0,songs: 0})

        if(respones && respones.changed) await delay(5);
        ress.push(respones)       
      }
      if (userid) {
        respones = await client.premium.ensure(userid, {
          enabled: false,
        })
        ...
        respones = await client.queuesaves.ensure(userid, {
        ...
        respones = await client.settings.ensure(userid, {
          dm: true,
        })
        ...
        respones = await client.stats.ensure(guildid + userid, {
          ban: [],
          kick: [],
          mute: [],
          ticket: [],
      ...
      if (userid && guildid) {
        ...
        respones = await client.userProfiles.ensure(userid, {
          id: userid,
          guild: guildid,
          totalActions: 0,
          warnings: [],
...
async function simple_databasing(client, guildid, userid) {
  ...
  try {
    if(guildid && userid){
      await client.stats.ensure(guildid+userid, {
        ban: [],
        kick: [],
        mute: [],
        ticket: [],
    ...
    if(userid){
      await client.settings.ensure(userid, { dm: true,}).catch(() => null);
    }
    if (guildid) { 
      
      await client.musicsettings.ensure(guildid, {"channel": "","message": ""}).catch(() => null);
      await client.stats.ensure(guildid, {commands: 0,songs: 0}).catch(() => null);
      ...
      await client.customcommands.ensure(guildid, {commands: []}).catch(() => null);
    }
    return;
  } catch (e) {
    console.error(e)
...
async function ensure_economy_user(client, guildid, userid){
    respones = await client.economy.ensure(`${guildid}_${userid}`, {
      user: userid,
      work: 0,
      balance: 0,
      bank: 0,
