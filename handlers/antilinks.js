                    ...
                    if (antireg.test(message.content) === true) {
                        ...
                        if(!usedLinks.some(link => !isAllowedUrl(link, antisettings))) {
                            ...
                            if (message.content.includes("```")) {
                                try {
                                    message.author.send(message.content.substring(0, 2000)).catch(() => null)
                                    message.author.send(`It seems like you sent a code in ${message.channel} **${message.guild.name}**, but it contains a Link, which isn't allowed!`).catch(() => null)
                                } catch (e) {
