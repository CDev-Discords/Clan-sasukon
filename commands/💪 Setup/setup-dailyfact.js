    ...
    try {
      ...
      async function first_layer(){
        ...
        let MenuEmbed = new MessageEmbed()
          .setColor(es.color)
          .setAuthor(client.getAuthor('Daily Facts Setup', 'https://emojipedia-us.s3.dualstack.us-west-1.amazonaws.com/thumbs/120/apple/285/page-with-curl_1f4c3.png', 'https://discord.gg/milrato'))
      ...
      async function handle_the_picks(optionhandletype, SetupNumber, menuoptiondata) {
        switch (optionhandletype) {
          case "Enable Daily fact":
            {
              ...
              await tempmsg.channel.awaitMessages({filter: m => m.author.id == message.author?.id, 
                ...
                .then(async collected => {
                  ...
                  if(channel){
                    await client.settings.set(message.guild.id +`.dailyfact`, channel.id)
          ...
          case "Disable Daily fact":
            {
              await client.settings.set(message.guild.id+`.dailyfact`, "no")
              return message.reply({embeds: [new Discord.MessageEmbed()
          ...
          case "Show Settings":
            {
              let thesettings = await client.settings.get(message.guild.id+`.dailyfact`)
              return message.reply({embeds: [new Discord.MessageEmbed()
